...
async function scrapeLinkedIn() {
  try {
    // Crear un archivo batch temporal para lanzar Chrome
    console.log('Creando archivo batch para lanzar Chrome...');
    const batchPath = path.join(process.cwd(), 'launch-chrome.bat');
    const batchScript = `
@echo off
...
exit
    ...
    console.log('Lanzando Chrome con script batch...');
    execSync(`cmd.exe /c "${batchPath}"`, { stdio: 'inherit' });
    
    // Esperar a que Chrome se inicie
    console.log('Esperando a que Chrome se inicie...');
    await new Promise(resolve => setTimeout(resolve, 5000));
    
    // Conectar Puppeteer al Chrome en ejecución
    ...
    // Desconectar pero no cerrar el navegador
    await browser.disconnect();
    console.log('Desconectado de Chrome. El navegador permanece abierto.');
    console.log('Puedes cerrar Chrome manualmente cuando hayas terminado.');
